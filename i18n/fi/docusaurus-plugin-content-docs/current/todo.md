---
sidebar_position: 2
---

# Osa 2 - TODO

Mitä tulossa seuraavaksi:

## Kokoelmasivut:

Lisättävä:
- CV
- Badget
- Harrastukset
	
## Yleistä parannusta sivustoon
- Yleinen verkkosivun ulkoasu ja alkuperäisen templaten poisto
- Etusivu
- Hakupalkki
- Työkaluja sivuille
    - hydra
    - john

# Suunnitelma tästä eteenpäin

Seuraavaksi jatketaan sivuprojektia ja muita projekteja blogia päivittäen.

## Projekteja käynnissä:

* [ ] 10-sormijärjestelmän opettelu, Osassa 14/15 
* [ ] Protocol Processing and Security @UTU
* [ ] Jotain COBOL kursseja @Youtube 0/3
* [ ] Google IT Support Professional Certificate
* [ ] Duolingo, Ruotsi ja Saksa, päivä 599 
* [ ] Microsoft Azure mini degree @edukamu
* [ ] Knowledge and Innovation Management @UTU
* [ ] System and Application Security @UTU
* [ ] Management of Information System Security @UTU
* [ ] Vanhat THM polut ja työkalut sivuille
 
## Valmiit projektit:
* [x] Työnhaku tietoturvan alalla
* [x] Sivun julkaisu
* [x] InfoSec - Computer Forensics Specialization
* [x] Try Hack Me SOC Level 1 -polku
* [x] Linux Server Course @Youtube
* [x] Try Hack Me SOC Level 2 -polku

### Voisi ehkä aloittaa:

Ei uutta ennen kun joku vanhoista on valmista:

- Microsoft & FreeCodeCamp C# Cert @Microsoft Learn
- IBM and ISC2 Cybersecurity Specialist Professional Certificate @Coursera
- (ISC)² Systems Security Certified Practitioner (SSCP) @Coursera
- Microsoft Cybersecurity Analyst Professional Certificate @Coursera
- Cyber Incident Response Specialization - InfoSec @Coursera
- [sysadmin course](https://www.cbtnuggets.com/it-training/skills/intro-systems-administration?utm_source=youtube.com&utm_medium=video&utm_campaign=freecodecamp&utm_content=description)
- Google Cybersecurity Professional Certificate @Coursera